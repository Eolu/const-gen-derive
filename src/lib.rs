...
fn impl_macro(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let val_impl: proc_macro2::TokenStream = match &ast.data {
        ...
        syn::Data::Union(data) => {
            let vis = get_field_visibilities(&data.fields.named)
                ...
                .next()
                .unwrap();
            let ident = get_field_idents(&data.fields.named)
                .into_iter()
                .next()
                .unwrap();
            quote! {
    ...
    let def_impl: proc_macro2::TokenStream = match &ast.data {
        ...
        syn::Data::Enum(data) => enum_def_handler(
            name,
            generics,
...
fn enum_val_handler(
    ...
    match fields {
        ...
        syn::Fields::Unnamed(f) => {
            let mut counter = 0;
            let new_idents: Vec<syn::Ident> = f
                .unnamed
                .iter()
                .map(|_| {
                    let new_ident = syn::Ident::new(&format!("idnt{}", counter), Span::call_site());
...
fn get_field_docs(
    ...
    fields
        .iter()
        .map(|field| get_inner_docs(&field.attrs, inner_doc_attr))
        .collect()
}
...
fn get_docs(attrs: &[syn::Attribute]) -> (Option<syn::Attribute>, bool) {
    ...
    if attrs.iter().any(|assoc_attr| {
        ...
        (
            attrs
                .iter()
                .find(|assoc_attr| assoc_attr.path.is_ident("doc"))
                .cloned(),
            inner_doc_attr,
        )
    } else {
